    ...
    <div class="container">
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <!-- click 后面可以是方法，也可以是表达式 -->
              <li :class="{'check':checkIndex==index}" 
                   v-for="(item,index) in addressFilter" 
                  :key="item.addressId" @click="checkIndex=index">
                <dl>
                  <dt>{{item.userName}}</dt>
          ...
          <div class="shipping-addr-more">
            <a class="addr-more-btn up-down-btn" href="javascript:;" @click="lookMore" :class="{'open':limit>3}">
              查看更多
              <i class="i-up-down">
        ...
        <div class="next-btn-wrap">
          <a class="btn btn--m btn--red" href="javascript:;" @click="modalConfirm=true">下一步</a>
        </div>
      </div>
    </div>
  </div>
     <nav-footer></nav-footer>
      ...
      <template v-slot:btnGroup>
        <!-- <a class="btn btn--m" href="javascript:;" @click="delAddress(item.addressId)">确认</a> -->
...
export default {
 ...
 components:{
  ...
  async mounted() {
    let {data} = await this.axios.get('/mock/address.json');
    let list = data.data;
